<script lang="ts">
	...
	$: performers = [
		...new Set(sketches.flatMap((s) => s.character_performers || []).map((cp) => cp.performer_name))
	].sort();

	$: filteredSketches = selectedPerformer
		? sketches.filter((s) =>
				s.character_performers?.some((cp) => cp.performer_name === selectedPerformer)
			)
		: sketches;

	$: printViewFilteredSketches = printViewPerformer
		? sketches.filter((s) =>
				s.character_performers?.some((cp) => cp.performer_name === printViewPerformer)
			)
		: sketches;
	...
	async function handleCreate(
		e: CustomEvent<{ sketch: Omit<Sketch, 'id' | 'created_at' | 'updated_at'> }>
	) {
		try {
	...
	async function handleImport(
		e: CustomEvent<{ importType: 'sketches' | 'techDetails' | 'unknown'; data: any[] }>
	) {
		try {
			...
			if (!response.ok) {
				const errorResult = await response
					.json()
					.catch(() => ({ error: 'Failed to import data. Unknown error.' }));
			...
			if (importType === 'sketches') {
				...
				if (newSketches.length > 0) {
					...
					if (errors.length > 0) {
						alert(
							`Import partially failed. ${errors.length} sketch(es) had issues. Check console for details.`
						);
						console.error('Sketch import errors:', errors);
					} else {
						alert(
							'No new sketches were imported. The file might have been empty or all rows failed.'
						);
					}
				}
			} else {
				...
				alert(
					`Tech details import complete. ${successfulTechUpdates} successful, ${failedTechUpdates} failed. Check console for details.`
				);
				if (failedTechUpdates > 0) {
					console.error(
						'Tech detail import issues:',
						importResults.filter((r: { success: boolean }) => !r.success)
					);
				}
	...
	async function handleUpdate(e: CustomEvent<{ sketch: Sketch }>) {
		try {
			...
			sketches = sketches
				.map((s) => (s.id === e.detail.sketch.id ? e.detail.sketch : s))
	...
	async function handleDrop(e: DragEvent, index: number) {
		...
		if (draggedIndex !== -1) {
			...
			const finalFilteredPositionMap = new Map(
				finalFilteredOrder.map((item, index) => [item.id, index])
			);
	...
	<div class="show-page">
		...
		<div class="content">
			<ViewControls
				...
				bind:printVersion
				bind:showPrintView
				on:viewModeChange={(e: CustomEvent<{ viewMode: 'grid' | 'list' }>) =>
					(viewMode = e.detail.viewMode)}
				on:printVersionChange={(
					e: CustomEvent<{ printVersion: 'greenroom' | 'hallway' | 'techbooth' }>
